...
import { updateConsumable, deleteConsumable } from '@/modules/logistics/service'
import { updateConsumableSchema } from '@/modules/logistics/validation'
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const item = await updateConsumable(params.id, user.id, parsed.data)
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const user = await requireAuth()
    await deleteConsumable(params.id, user.id)

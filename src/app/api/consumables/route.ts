...
import { createConsumable } from '@/modules/logistics/service'
import { createConsumableSchema } from '@/modules/logistics/validation'
import { z } from 'zod'

const schema = createConsumableSchema.extend({ listingId: z.string().min(1) })
...
export async function POST(req: NextRequest) {
  try {
    ...
    const item = await createConsumable(listingId, user.id, input)

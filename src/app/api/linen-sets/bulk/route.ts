...
import { bulkUpdateLinenState } from '@/modules/logistics/service'
import { bulkLinenStateSchema } from '@/modules/logistics/validation'
import { z } from 'zod'

const schema = bulkLinenStateSchema.extend({ listingId: z.string().min(1) })
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const result = await bulkUpdateLinenState(listingId, user.id, input)

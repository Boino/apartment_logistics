...
import { updateLinenState, deleteLinenSet } from '@/modules/logistics/service'
import { updateLinenStateSchema } from '@/modules/logistics/validation'

export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const user = await requireAuth()
    const body = await req.json().catch(() => ({}))
    const parsed = updateLinenStateSchema.safeParse(body)
    if (!parsed.success) {
    ...
    const set = await updateLinenState(params.id, user.id, parsed.data)
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const user = await requireAuth()
    await deleteLinenSet(params.id, user.id)
